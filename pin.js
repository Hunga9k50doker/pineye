...
class PinEye {
  ...
  async claimQuest(token, questId, answerId) {
    ...
    try {
      ...
      this.log(`Không thể claim quest ${questId}: ${error.message}`, "error");
      return null;
    }
  }
  ...
  async handleDailyCombo(token) {
    ...
    try {
      ...
      this.log(`Không thể handle DailyCombo: ${error.message}`, "error");
      return null;
    }
  }
  ...
  async claimPratice(token, id) {
    ...
    try {
      ...
      this.log(`Không thể claim Pratice: ${error.message}`, "error");
      return null;
    }
  }
  ...
  async handlePractice(token) {
    ...
    if (practices && practices?.length > 0) {
      ...
      if (tasks?.length > 0) {
        ...
        this.log(`You completed practice today or no day to practice!`, "warning");
      }
    }
    return;
  }
